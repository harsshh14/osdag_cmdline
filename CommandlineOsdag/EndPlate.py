...
from CommandlineOsdag.design_type_v2.connection.end_plate_connection import EndPlateConnection
# from UI_DESIGN_PREFERENCE import DesignPreferences
from Common import *
from utils.common.other_standards import PATH_TO_DATABASE
colorama.init(autoreset=True)
class EndPlate(EndPlateConnection):
    def __init__(self,**kwargs):
        super().__init__()
        self.design_dict={}
        self.design_pref_inputs={}
        self.design_pref()
        self.input_dock_inputs = EndPlateConnection.input_values(self)
    ...
    def design_pref(self):
        option_list = EndPlateConnection.input_values(self)
        new_list = EndPlateConnection.customized_input(self)
        updated_list = EndPlateConnection.input_value_changed(self)
        out_list = EndPlateConnection.output_values(self,False)
        data = {}
        last_design_folder = os.path.join(r'../ResourceFiles', 'last_designs')
        last_design_file = str(EndPlateConnection.module_name(self)).replace(' ', '') + ".osi"
        last_design_file = os.path.join(last_design_folder, last_design_file)
        last_design_dictionary = {}
        print(last_design_folder)
    ...
    def design_fn(self, op_list, data_list):
        ...
        #     print('flag true')
            ...
            for without_des_pref in EndPlateConnection.input_dictionary_without_design_pref(self):
                ...
                for key_name in input_list:
                    if input_source == 'Input Dock':
                        ...
                        val = EndPlateConnection.get_values_for_design_pref(self,key_name, design_dictionary)
                        design_dictionary.update({key_name: val})
    ...
    def input(self):
        ...
        self.design_dict['Material'] = self.material        # Connectivity Column Flange-Beam Web
    ...
    def set_inputs(self):
        ...
        EndPlateConnection.set_input_values(self,self.design_inputs)
    ...
    def outputs(self):
        ...
        print(Fore.GREEN+"0. Enter Again")
    ...
    def show_output(self):
        ...
        outputs= EndPlateConnection.output_values(self,self.design_status)
